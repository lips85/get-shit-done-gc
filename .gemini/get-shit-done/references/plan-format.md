<overview>
Gemini-executable plans have a specific format that enables Gemini to implement without interpretation. This reference defines what makes a plan executable vs. vague.

**Key insight:** PLAN.md IS the executable prompt. It contains everything Gemini needs to execute the phase, including objective, context references, tasks, verification, success criteria, and output specification.
</overview>

<core_principle>
A plan is Gemini-executable when Gemini can read the PLAN.md and immediately start implementing without asking clarifying questions.

If Gemini has to guess, interpret, or make assumptions - the task is too vague.
</core_principle>
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>[what Gemini automated]</what-built>
...
<type name="auto">
**Default task type** - Gemini executes autonomously.
...
Use for: Everything Gemini can do independently (code, tests, builds, file operations).
</type>

<type name="checkpoint:human-action">
**RARELY USED** - Only for actions with NO CLI/API. Gemini automates everything possible first.
...
**Execution:** Gemini automates everything with CLI/API, stops only for truly unavoidable manual steps.
</type>

<type name="checkpoint:human-verify">
**Human must verify Gemini's work** - Visual checks, UX testing.
...
**Execution:** Gemini builds the feature, stops, provides testing instructions, waits for approval/feedback.
</type>
...
**Execution:** Gemini presents options with balanced pros/cons, waits for decision, proceeds with chosen direction.
</type>

**When to use checkpoints:**

- Visual/UX verification (after Gemini builds) → `checkpoint:human-verify`
...
- Anything with CLI/API (Gemini automates it) → `type="auto"`
...
**Golden rule:** If Gemini CAN automate it, Gemini MUST automate it.
...
Reference files that Gemini needs to understand before implementing.
</context_references>
...
Gemini: "How? What type? What library? Where?"
</too_vague>
...
Gemini can implement this immediately.
</just_right>
...
<too_detailed>
Writing the actual code in the plan. Trust Gemini to implement from clear instructions.
</too_detailed>
</specificity_levels>
...
These require Gemini to decide WHAT to do. Specify it.
</vague_actions>
...
Gemini doesn't know your standards. Be explicit.
</missing_context>
</anti_patterns>

<sizing_tasks>
Good task size: 15-60 minutes of Gemini work.
